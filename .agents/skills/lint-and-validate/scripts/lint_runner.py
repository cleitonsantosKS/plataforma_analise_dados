...
def detect_project_type(project_path: Path) -> dict:
    ...
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        ...
        # Check for ruff in virtual environment
        ruff_win = project_path / ".venv" / "Scripts" / "ruff.exe"
        ruff_unix = project_path / ".venv" / "bin" / "ruff"
        if ruff_win.exists():
            result["linters"].append({"name": "ruff", "cmd": [str(ruff_win), "check", "."]})
        elif ruff_unix.exists():
            result["linters"].append({"name": "ruff", "cmd": [str(ruff_unix), "check", "."]})
        else:
